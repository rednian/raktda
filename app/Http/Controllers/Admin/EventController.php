            ...
            switch ($request->status) {
                ...
                case 'need modification':
                    ...
                    if ($request->requirements_id) {
                        $requirements_id = array_filter($request->requirements_id, function ($v) {
                            if (!empty($v)) {
                                return ($v);
                            }
                        });
        ...
        return DataTables::of($requirements)
            ->addColumn('name', function ($requirement) use ($request) {
                if ($request->user()->LanguageId == 1) {
                    return ucfirst($requirement->requirement_name);
                }
    ...
    public function dataTable(Request $request)
    {
        if ($request->ajax()) {
            ...
            $table = DataTables::of($events)
                ...
                ->addColumn('expected_audience', function ($event) {
                    return $event->audience_number;
                })
                ...
                ->addColumn('has_liquor', function ($event) {
                    return $event->liquor()->exists() ? __('YES') : __('NO');
                })
                ->addColumn('has_truck', function ($event) {
                    return $event->truck()->exists() ? __('YES') : __('NO');
                })
                ->addColumn('has_artist', function ($event) {
                    return $event->permit()->exists() ? $event->permit->artistPermit()->count() : __('NO');
                })
                ...
                ->addColumn('event_name', function ($event) use ($user) {
                    if ($user->LanguageId == 1) {
                        return ucfirst($event->name_en);
                    }
                    return ucfirst($event->name_ar);
                })
