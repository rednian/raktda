...
class ArtistController extends Controller
{
    ...
    public function reset_req_in_session(Request $request)
    {
        $reqId = $request->id;
    ...
    {
        $user_id = Auth::user()->user_id;
        session()->forget([$user_id . '_pic_file', $user_id . '_thumb_file']);
        ...
        if ($edit == 0) {
            ...
            foreach ($permit_details->artistPermit as $pd) {
                ...
                $artist_temp = ArtistTempData::create([
                    ...
                    'work_location_ar' => $permit_details->work_location_ar,
    ...
    public function edit_artist(Request $request , $id, $from)
    {
        ...
        if ($from == 'amend') {
            ...
            // if(!$data_bundle['artist_details']){
            //     return redirect(URL::signedRoute('artist.index').'#valid');
            // }
        } else {
