...
//            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($evd['isLiquor'] == 1) {
                EventLiquor::whereNull('event_id')->where('status', 0)->where('created_by', Auth::user()->user_id)->orderBy('created_at', 'desc')
                    ->take(1)->update([
                        'event_id' => $event_id,
                    ]);
    ...
    {
        return  EventTruck::where('event_id', $id)->whereIn('status', [0 , 2 ])->get();
    }
    ...
    public function update_event(Request $request)
    {
        try {
            ...
            $eventData = Event::where('event_id', $event_id)->first();

            if (is_null($eventData->liquor->company_name_en) && $evd['isLiquor'] == 1) {
                EventLiquor::whereNull('event_id')->where('status', 0)->where('created_by', Auth::user()->user_id)->orderBy('created_at', 'desc')
                    ->take(1)->update([
                        'event_id' => $event_id,
                    ]);
            ...
            switch ($status) {
                ...
                case 'valid':
                    if ($permit->status == 'active') {
                        ...
                        $amend_btn = ($diff <= $amend_grace) ? '<a href="' . \Illuminate\Support\Facades\URL::signedRoute('event.amend', $permit->event_id) . '" ><span class="kt-badge kt-badge--warning kt-badge--inline kt-margin-l-15">' . __('Amend') . '</span></a><a href="' . \Illuminate\Support\Facades\URL::signedRoute('event.add_artist', $permit->event_id) . '" title="' . __('Add Artist') . '" class="kt-font-dark kt-pull-right"><i class="fa fa-user-plus"></i></a><br />' : '';
    ...
    public function amend(Request $request, Event $event)
    {
        ...
        if(Str::contains($previousPageUrl, 'http://raktda.test/company/event?')){


            $eventTrucks = EventTruck::where([
                ['event_id', $event->event_id],
                ['status', 2]
            ]);

            $eventTrucksArray =  $eventTrucks->get()->toArray();


            $truckIds = Arr::pluck($eventTrucksArray, 'event_truck_id');

            for($i = 0; $i < count($truckIds) ; $i++){
                EventLiquorTruckRequirement::where('liquor_truck_id', $truckIds[$i])->delete();
            }

            $eventTrucks->delete();


//            $eventliquor = EventLiquor::where([
//                ['event_id', $event->event_id],
//                ['status', 2]
//            ]);
//
//            dd($eventliquor);
        ...
        $data['tab'] = 'valid';
        $data['truck_req'] = Requirement::where('requirement_type', 'truck')->get();
        $truck_details = $event->truck()->get();


        $data['liquor_req'] = Requirement::where('requirement_type', 'liquor')->get();
        $isLiquorUnpaid = $event->liquor()->where('paid', 0)->exists();

        $isTruckUnpaid = $truck_details->contains(function ($value, $key) {
            return $value['paid'] != 1;
        });

        $data['isUnpaid'] =  $isTruckUnpaid || $isLiquorUnpaid ;

        $data['emirates'] = Emirates::orderBy('name_en')->get();


        return view('permits.event.amend', $data);
    }

    public function deleteFoodTruck(Request $request) {
        $truck_id = $request->del_truck_id;
        $event_id = $request->del_truck_event_id;
    ...
    public function applyAmend(Request $request)
    {
        try {
            ...
            $type = $request->type;
            $event_id = $request->event_id;
            $event = Event::where('event_id', $event_id)->update(
                [
...
//            }
    ...
    public function fetch_requirements(Request $request)
    {
        ...
        })->where('type', $firm)->where('requirement_type', 'event')->with(['event_type_requirements' => function ($q) use ($id) {
