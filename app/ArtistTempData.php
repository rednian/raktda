...
class ArtistTempData extends Model
{
    ...
    protected $fillable = [
        'artist_id', 'permit_id', 'permit_type_id', 'original', 'thumbnail', 'sponsor_name_ar', 'sponsor_name_en', 'visa_expire_date', 'visa_number', 'visa_type', 'language', 'visa_type', 'mobile_number', 'email', 'fax_number', 'po_box', 'phone_number', 'address_ar',  'city', 'address_en', 'area', 'passport_expire_date', 'passport_number', 'uid_expire_date', 'religion',  'emirates_id', 'uid_number',  'firstname_en', 'firstname_ar', 'lastname_en', 'lastname_ar', 'nationality', 'birthdate', 'gender',  'status', 'permit_type_name', 'artist_permit_id', 'person_code', 'is_old_artist', 'profession_id', 'artist_permit_status', 'issue_date', 'expiry_date', 'work_location', 'work_location_ar', 'created_by', 'company_id', 'process', 'permit_number', 'del_status', 'event_id','old_artist_id','type', 'is_paid'
    ];


    public function ArtistTempDocument()
    {
        return $this->hasMany(ArtistTempDocument::class, 'temp_data_id', 'id');
    }
