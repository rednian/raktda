...
<section class="kt-portlet kt-portlet--last kt-portlet--responsive-mobile" id="kt_page_portlet">
  <div class="kt-portlet__body">
    ...
    <section class="row">
      <div class="col-md-12">
        <ul class="nav nav-tabs nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-danger"
          ...
          <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#checked"
              data-target="#checked">{{ __('Checked Permits') }}</a></li>
        </ul>
      </div>
    </section>
    <div class="tab-content">
      ...
      <div class="tab-pane show fade" id="checked" role="tabpanel">
        <section class="form-row">
          <div class="col-1">
            <div>
              <select name="length_change" id="checked-length-change"
                class="form-control-sm form-control custom-select custom-select-sm" aria-controls="artist-permit">
                <option value='10'>10</option>
                <option value='25'>25</option>
                <option value='50'>50</option>
          ...
          <div class="col-8">
            <form class="form-row">
              ...
              <div class="col-3">
                <select name="" id="checked-applicant-type"
                  class="form-control-sm form-control custom-select custom-select-sm " onchange="checkedTable.draw()">
                  <option selected disabled>{{ __('APPLICATION TYPE') }}</option>
          ...
          <div class="col-md-3">
            <div class="form-group form-group-sm">
              <div class="kt-input-icon kt-input-icon--right">
                <input autocomplete="off" type="search" class="form-control form-control-sm"
                  placeholder="{{ __('Search') }}..." id="search-checked-request">
                <span class="kt-input-icon__icon kt-input-icon__icon--right">
                  <span><i class="la la-search"></i></span>
                </span>
              </div>
        ...
        <table class="table table-hover table-borderless table- border table-striped" id="checked-request">
          <thead>
            <tr>
              <th>{{ __('REFERENCE NO.') }}</th>
              <th>{{ __('ESTABLISHMENT NAME') }}</th>
    ...
    var checkedTable;
     ...
     function checkedEvents() {
      ...
      $('input#checked-applied-date').daterangepicker({
       ...
       checkedTable = $('table#checked-request').DataTable({
        dom: "<'row d-none'<'col-sm-12 col-md-6 '><'col-sm-12 col-md-6'>>" +
              "<'row'<'col-sm-12'tr>>" +
              "<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>",
         ajax: {
           ...
           data: function (d) {
            ...
            var status = $('select#checked-permit-status').val();
             d.status =  ['need approval', 'checked'];
             d.type = $('select#checked-applicant-type').val();
             d.date = $('#checked-applied-date').val()  ? selected_date : null;
             d.checked = 1;
             @if(Auth::user()->roles()->first()->role_id == 6)
             d.gov = 1;
             @endif
       ...
       //clear fillte button
        $('#checked-btn-reset').click(function(){ $(this).closest('form.form-row')[0].reset(); checkedTable.draw();});
       //custom pagelength
       checkedTable.page.len($('#checked-length-change').val());
       $('#checked-length-change').change(function(){ checkedTable.page.len( $(this).val() ).draw(); });
       //custom search

       var search = $.fn.dataTable.util.throttle(function(v){ checkedTable.search(v).draw(); });
       $('input#search-checked-request').keyup(function(){ if($(this).val() == ''){ } search($(this).val()); });
     }
</script>
@endsection
