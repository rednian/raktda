	...
	<section class="kt-portlet  kt-portlet--head-sm kt-portlet--responsive-mobile">
		<div class="kt-portlet__head kt-portlet__head--sm kt-portlet__head--noborder">
			 <div class="kt-portlet__head-label">
					<h3 class="kt-portlet__head-title kt-font-transform-u kt-font-dark">{{ is_null($user->NameEn) ? __('Edit Time Off') :  __('Edit Time Off') . ' - ' . (Auth::user()->LanguageId == 1 ? $user->NameEn : $user->NameAr) }}</h3>
			 </div>
		...
		<div class="kt-portlet__body kt-padding-t-0">
			<form method="POST" id="formAddLeave" action="{{ route('user_management.leave.update', ['leave' => $leave->employee_leave_id, 'user' => $user->user_id]) }}">
			@csrf
                ...
                <div class="col-sm-6">
                    <div class="form-group form-group-sm">
                        ...
                        <select required name="user_id" id="kt_select2_1" class="form-control">
							...
							<option {{ $emp->user_id == $leave->user_id ? 'selected' : '' }} value="{{ $emp->user_id }}">{{ Auth::user()->LanguageId == 1 ? $emp->NameEn : $emp->NameAr }}</option>
                        	@endforeach
                        </select>
                    </div>
                </div>
				...
				<div class="col-sm-6">
                    <div class="form-group form-group-sm">
                        ...
                        <select required name="leave_type_id" class="form-control">
							...
							<option {{ $leave->leave_type_id == $type->leave_type_id ? 'selected' : '' }} value="{{ $type->leave_type_id }}">{{ Auth::user()->LanguageId == 1 ? $type->leave_type_name : $type->leave_type_name_ar }}</option>
                        	@endforeach
                        </select>
                    </div>
                </div>
                    	...
                    	<section class="row kt-margin-t-10">
			                <div class="col-sm-6">
			                	<div class="form-group">
				                    ...
				                    <div class="input-group date">
										<input value="{{ date('m/d/Y h:i A', strtotime($leave->leave_start)) }}" required name="leave_start" type="text" class="form-control" id="kt_datetimepicker_start" placeholder="Select date and time"/>
			                ...
			                <div class="col-sm-6">
			                	<div class="form-group">
				                    ...
				                    <div class="input-group date">
										<input value="{{ date('m/d/Y h:i A', strtotime($leave->leave_end)) }}" required name="leave_end" type="text" class="form-control" id="kt_datetimepicker_end" placeholder="Select date and time"/>
										<div class="input-group-append">
											<span class="input-group-text">
												<i class="la la-calendar glyphicon-th"></i>
											</span>
            ...
            <section class="row kt-margin-t-10">
                <div class="col-sm-12">
                    ...
                    <textarea required class="form-control form-control-sm" name="remarks" rows="5">{{ $leave->remarks }}</textarea>
                </div>
            </section>
            <input type="hidden" name="submit_type">
        	</form>

        	<form method="POST" action="{{ route('user_management.leave.delete', ['leave' => $leave->employee_leave_id, 'user' => $user->user_id]) }}" id="formDeleteLeave">
        		@csrf
        	</form>
		</div>
	</section>
