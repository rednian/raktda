...
@section('content')
	<section class="kt-portlet  kt-portlet--head-sm kt-portlet--responsive-mobile">
		...
		<div class="kt-portlet__body kt-padding-t-0">
			<form method="POST" id="formAddScheduleType" action="{{ route('schedule_type.update', $schedule_type->schedule_type_id) }}">
			@csrf
			@method('patch')
			...
			<section class="row kt-margin-t-10">
                <div class="col-sm-6">
                    <div class="form-group form-group-sm">
                        ...
                        <input value="{{ $schedule_type->schedule_type_name }}" type="text" name="schedule_type_name" required class="form-control form-control-sm">
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group form-group-sm">
                        ...
                        <input value="{{ $schedule_type->schedule_type_name_ar }}" type="text" name="schedule_type_name_ar" required class="form-control form-control-sm">
                    </div>
                </div>
            </section>
                    	...
                    	<section class="row kt-margin-t-10">
			                <div class="col-sm-3">
			                    <div class="form-group form-group-sm">
			                        ...
			                        <input value="{{ $schedule_type->time_start }}" type="time" id="time_start" class="form-control form-control-sm pre_time">
			                    </div>
			                </div>
			                <div class="col-sm-3">
			                    <div class="form-group form-group-sm">
			                        ...
			                        <input value="{{ $schedule_type->time_end }}" type="time" id="time_end" class="form-control form-control-sm pre_time">
			                    </div>
			                </div>
			                <div class="col-sm-3">
			                    <div class="form-group form-group-sm">
			            ...
			            <section class="row kt-margin-t-10">
			                <div class="col-sm-12">
			                    <table class="table table-borderless table-striped table-hover border" id="schedule-type-table">
									...
									<tbody>
										@foreach($schedule_type->getSchedule as $day)
										<tr>
											<th>{{ __($day->day) }}</th>
											<td class="text-center">
												<div class="form-group form-group-sm">
							                        <label class="kt-checkbox {{ $day->is_dayoff ? 'kt-checkbox--success' : 'kt-checkbox--default' }} kt-font-dark">
														<input class="is_dayoff" data-day="{{ $day->schedule_type_daytime_id }}" {{ $day->is_dayoff ? 'checked' : '' }} name="day[{{ $day->schedule_type_daytime_id }}][is_dayoff]" value="1" type="checkbox">
														<span></span>
													</label>
							                    </div>
											</td>
											<td>
												<div class="form-group form-group-sm">
							                        <input value="{{ $day->time_start }}" {{ $day->is_dayoff ? 'disabled' : '' }} type="time" name="day[{{ $day->schedule_type_daytime_id }}][time_start]" id="time_start_{{ $day->schedule_type_daytime_id }}" required class="form-control form-control-sm day_time_start">
							                    </div>
											</td>
											<td>
												<div class="form-group form-group-sm">
							                        <input value="{{ $day->time_end }}" {{ $day->is_dayoff ? 'disabled' : '' }} type="time" name="day[{{ $day->schedule_type_daytime_id }}][time_end]" id="time_end_{{ $day->schedule_type_daytime_id }}" required class="form-control form-control-sm day_time_end">
							                    </div>
											</td>
										</tr>
										@endforeach
...
@section('script')
    ...
    $(document).ready(function () {
		...
		var validated_form = $('form#formAddScheduleType').validate({
     		rules:{
     			@foreach($schedule_type->getSchedule as $day)
     			'day[{{ $day->schedule_type_daytime_id }}][time_start]': {
				    lessThan: "#time_end_{{ $day->schedule_type_daytime_id }}"
				},
     			'day[{{ $day->schedule_type_daytime_id }}][time_end]': {
				    greaterThan: "#time_start_{{ $day->schedule_type_daytime_id }}"
				},
     			@endforeach
     		}
     	});
