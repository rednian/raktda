...
@section('title', 'View Event Permit Details - Smart Government Rak')
                ...
                <div class="col-md-7">
                    <table class="table table-borderless table-sm">
                        <tr class="kt-margin-b-0 kt-font-dark">
                            <td class="kt-font-bold kt-margin-r-15">{{__('Applicant Type')}}</td>
                            <td>:</td>
                            <td>{{__(ucfirst($event->firm))}}
                            </td>
                        </tr>
                        <tr class="kt-margin-b-0 kt-font-dark">
                            <td class="kt-font-bold kt-margin-r-5">{{__('Event Name')}}</td>
                            <td>:</td>
                            <td>{{getLangId() == 1 ? ucfirst($event->name_en) : $event->name_ar}}</td>
                        </tr>
                        <tr class="kt-margin-b-0 kt-font-dark">
                            <td class="kt-font-bold kt-margin-r-5">{{__('Event Owner')}}</td>
                            <td>:</td>
                            <td> {{getLangId() == 1 ?  ucfirst($event->owner_name) : $event->owner_name_ar }}</td>
                        </tr>
                        <tr class="kt-margin-b-0 kt-font-dark">
                            <td class="kt-font-bold kt-margin-r-15">{{__('Event Type')}}</td>
                            <td>:</td>
                            <td>{{!empty($event->type) ? getLangId() == 1 ? ucfirst($event->type->name_en) : $event->type->name_ar : ''}}
                            </td>
                        </tr>
                        @if($event->subType)
                        <tr class="kt-margin-b-0 kt-font-dark">
        ...
        <div class="kt-widget__body kt-margin-t-10">
            ...
            <table class="table table-sm table-hover table-borderless table-display">
                ...
                <tr>
                    ...
                    <td>:</td>
                    <td class="kt-font-dark">{{ucfirst($event->cancel_reason)}}</td>
                </tr>
                @endif
                {{--
                            <tr>
...
<div>
    ...
    <div class="table-responsive">
        <table class="table table-borderless border table-striped">
            ...
            <tbody id="food_truck_list">
                ...
                <tr>
                    <td>{{ucfirst($truck->company_name_en)}}</td>
    ...
    <div class="table-responsive">
        <table class="table table-borderless border table-striped">
            ...
            <tbody id="food_truck_list">
                <tr>
                    <td>{{ucfirst($liquor->company_name_en)}}</td>
...
<div class="pt-2 img-responsive">
    ...
    <table class="table table-striped table-hover border table-borderless">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ getLangId() == 1 ? ucfirst($at->firstname_en) : $at->firstname_ar}}</td>
                <td>{{ getLangId() == 1 ? ucfirst($at->lastname_en) : $at->lastname_ar}}</td>
...
<div class="event--requirement-files">
    <table class="table table-hover table-borderless border table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td colspan="4">
                    <strong>{{getLangId() == 1 ? ucfirst($req->requirement_name) : $req->requirement_name_ar}}</strong>
                </td>
            </tr>
            @endif
            <tr>
                <td style="width:50%;">
                    {{getLangId() == 1 ? ucfirst($req->requirement_name) : $req->requirement_name_ar}}&nbsp;{{'-'.strval($j + 1)}}
                </td>
                <td class="text-center">
...
<h5 class="text-dark kt-margin-b-15 kt-margin-t-15 text-underline kt-font-bold">{{__('Uploaded Images')}}</h5>

<div class="row col-md-12">
    @foreach($eventImages->reverse() as $i => $upload)
    {{-- {{ dd($eventImages)}} --}}
    <div class="col-md-3 my-4">
        <div class="container">
            ...
            <a id="single_image_{{$i + 1}}" href="{{url('storage').'/'.$upload->path}}"><img
                    src="{{url('storage').'/'.$upload->thumbnail}}" alt="" /></a>
...
@endforeach
<input type="hidden" id="imgCount" value={{$eventImages->count()}}>
</div>
@endif
